...
- **对话保存与恢复(轻量)**:顶栏保存按钮手动将当前对话(仅用户/助手文本)导出为库内笔记——不做实时同步,同一会话每次保存覆盖同一文件。笔记按可配置文件夹组织(默认 `YOLO/<YYYY-MM-DD>/<标题>.md`),通过内嵌的会话 id 标记去重。保存的笔记与 opencode 会话并列显示在历史列表中,选中后以新对话自动发起恢复(内置恢复提示词),适合在另一台设备上延续对话
...
- 对话保存为纯 Markdown 笔记并附带 `openyolo-session` 元数据标记,同一会话重复保存时即使标题变更也会覆盖原文件;恢复时笔记内容直接嵌入提示词发送,无需工具调用
